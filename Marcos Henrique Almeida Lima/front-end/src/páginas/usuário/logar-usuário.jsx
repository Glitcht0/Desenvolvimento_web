...
export default function LogarUsuário() {
  ...
  async function logarUsuário() {
    console.log("Iniciando tentativa de login para:", dados.nome_login);
    if (validarCampos()) {
      try {
        const response = await serviçoLogarUsuário(dados);
        console.log("Resposta da API de login recebida:", response);
        setUsuárioLogado({ ...response.data?.usuárioLogado, cpf: dados.nome_login,
          cadastrado: true });
        navegar("/pagina-inicial");
      } catch (error) {
        console.error("Erro na chamada da API de login:", error.response || error);
